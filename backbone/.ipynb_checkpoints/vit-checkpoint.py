...
class VisionTransformer(MammothBackbone):
    ...
    def forward(self, x: torch.Tensor, AB: dict = {}, returnt='features'):
        """
        Compute the forward pass of ViT.
        """
        assert returnt in ('out', 'features', 'both', 'full', 'cls_tokens')
    
        # Forward pass through the transformer blocks
        if returnt == 'cls_tokens':
            cls_tokens = []  # 用于存储每层的 cls_token
            features = self.forward_features(x, AB, return_all=True)  # 获取所有层的输出
    
            # 提取前12个block的 cls_token
            for feature in features[:-1]:  # 只取前12个张量
                cls_token = feature[:, 0, :]  # 假设 cls_token 是第一个位置
                cls_token = self.norm(cls_token)  # 对每一个 cls_token 进行 Layer Normalization
                cls_tokens.append(cls_token.clone())  # 克隆以避免引用问题
    
            # 将 cls_tokens 列表转换为张量，形状为 [depth, batch_size, embed_dim]
            cls_tokens = torch.stack(cls_tokens, dim=0)
            return cls_tokens
    
        # 如果不是 'cls_tokens' 选项，则按照原逻辑处理
